...
async function processBatch(items, batchName) {
  console.log(`\n${"=".repeat(50)}`);
  console.log(`${batchName} (${items.length}件)`);
  console.log(`${"=".repeat(50)}\n`);

  // 全ジョブを開始（2秒間隔）
  const jobs = [];
  for (const item of items) {
    const job = await startJob(item);
...
async function main() {
  ...
  totalSuccess += await processBatch(tiktokBatch, "TikTok収集");

  // Instagramバッチ
  const instagramBatch = COLLECTIONS.filter(c => c.platform === "instagram");
  totalSuccess += await processBatch(instagramBatch, "Instagram収集");

  // YouTubeバッチ
  const youtubeBatch = COLLECTIONS.filter(c => c.platform === "youtube");
  totalSuccess += await processBatch(youtubeBatch, "YouTube収集");
  ...
  console.log(`\n${"=".repeat(50)}`);
  console.log(`全処理完了: ${totalSuccess}件成功 (${elapsed}秒)`);
  console.log(`${"=".repeat(50)}`);
}
