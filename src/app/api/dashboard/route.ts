...
import { isJapaneseContent } from "@/lib/japan-filter";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const videoWhere: {
      ...
      videoTags?: { some: { industryId: number } };
      platform?: string;
    } = {
    ...
    // コンテンツタイプ別集計
    const contentTypeStats: Record<
      string,
      { count: number; totalViews: number; avgEngagement: number }
    > = {};
    const hookTypeStats: Record<
      string,
      { count: number; totalViews: number; avgEngagement: number }
    > = {};
    const durationCategoryStats: Record<
      string,
      { count: number; totalViews: number; avgEngagement: number }
    > = {};
      ...
      for (const tag of video.videoTags) {
        ...
        if (tag.contentType) {
          if (!contentTypeStats[tag.contentType]) {
            contentTypeStats[tag.contentType] = {
              count: 0,
              totalViews: 0,
              avgEngagement: 0,
            };
          ...
          contentTypeStats[tag.contentType].avgEngagement +=
        ...
        if (tag.durationCategory) {
          if (!durationCategoryStats[tag.durationCategory]) {
            durationCategoryStats[tag.durationCategory] = {
              count: 0,
              totalViews: 0,
              avgEngagement: 0,
            };
          ...
          durationCategoryStats[tag.durationCategory].totalViews +=
            video.viewCount;
          durationCategoryStats[tag.durationCategory].avgEngagement +=
            video.engagementRate;
        }
      }
    }
    ...
    for (const key of Object.keys(durationCategoryStats)) {
      durationCategoryStats[key].avgEngagement /=
        durationCategoryStats[key].count;
    }

    // 日別トレンド
    const dailyTrend: Record<
      string,
      { date: string; videos: number; views: number; engagement: number }
    > = {};
    for (const video of videos) {
      if (video.postedAt) {
